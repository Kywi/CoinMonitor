...
public class BinanceWebSocketManager : IWebSocketManager
{
    ...
    public async Task StartAsync()
    {
        ...
        {
            var subscription = new BinanceWebSocketSubscriptionDto
            {
                Method = "SUBSCRIBE",
    ...
    private async Task ReceiveAsync()
    {
        ...
        while (_socket.State == WebSocketState.Open)
        {
            ...
            else
            {
                ...
                var update = JsonConvert.DeserializeObject<BinanceTickerDto>(json);

                if (update?.Symbol != null)
                {
                    PriceUpdate?.Invoke(this, new PriceChangedEventArgs(update.Symbol, update.Price, "Binance"));
                }
            }
        }
    }
